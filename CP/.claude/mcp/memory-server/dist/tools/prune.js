...
export async function memoryPrune(input) {
    const { max_age_days = DEFAULT_MAX_AGE_DAYS, max_entries = DEFAULT_MAX_ENTRIES, dry_run = true, confirm = false } = input;
...
function formatPrunePreview(pruned, toRemove, maxAgeDays, maxEntries) {
...
function formatPruneConfirmation(pruned, maxAgeDays, maxEntries) {
    const total = Object.values(pruned).reduce((a, b) => a + b, 0);
    const lines = [
